...
def entry_feeds_list(request):
    ...
    if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
        ...
        paginator = Paginator(entry, 100)  
        page_number = request.GET.get('page')
        entry_page = paginator.get_page(page_number)
        context["entries"] = entry_page
...
def grower_payment_split_list(request):
    ...
    if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
        ...
        paginator = Paginator(grower_payee, 100)  
        page_number = request.GET.get('page')
        grower_payee_page = paginator.get_page(page_number)
        grower_payee_lst = []
        for i in grower_payee_page :
        ...
        context['page_obj'] = grower_payee_page
